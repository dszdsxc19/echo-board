...
class MemoryIngestionEngine:
    ...
    def process_file(self, file_content: str, source_name: str = "unknown") -> List[LifeEvent]:
        ...
        # 4. 存入仓库 & 记忆 (Parallel execution)
        # ⚡ Bolt Optimization: Run vector store ingestion and memory extraction in parallel
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            if life_events:
                ...
                futures.append(executor.submit(add_events_task))
            else:
                logger.warning(f"⚠️ 未从文件 {source_name} 中提取到有效内容")

            # Task B: User Profile Memory
            futures.append(executor.submit(self.mem0.remember, file_content))

            # Wait for all tasks to complete and raise exceptions if any
