...
class MemoryIngestionEngine:
    def __init__(self, knowledge_base: KnowledgeBase):
        ...
        # ⚡ Bolt Optimization: Initialize splitters once
        headers_to_split_on = [
            ("#", "Date/Title"),
            ("##", "Section"),
            ("###", "SubSection"),
    ...
    def process_file(self, file_content: str, source_name: str = "unknown") -> List[LifeEvent]:
        ...
        # 1. 结构化切分 (按标题)
        # ⚡ Bolt Optimization: Use pre-initialized splitter
        ...
        # 4. 存入仓库 & 5. Mem0 记忆
        # ⚡ Bolt Optimization: Parallel execution of independent I/O operations
        def save_events():
            if life_events:
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            future_events = executor.submit(save_events)
            future_mem0 = executor.submit(remember_content)

            # Wait for both to complete to ensure data integrity before returning
