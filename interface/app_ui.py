        ...
        if st.button("🔄 同步数据", disabled=not folder_path):
            if folder_path and os.path.exists(folder_path):
                ...
                try:
                    ...
                    # ⚡ Bolt Optimization: Parallel Processing
                    # We use ThreadPoolExecutor to process files in parallel.
                        ...
                        for future in concurrent.futures.as_completed(future_to_file):
                            ...
                            try:
                                ...
                                if result["status"] == "✅":
                                    ...
                                    # Update Current File Display (Note: this might flash quickly in parallel)
                                    ...
                                    # Error handling from helper
                                    processed_files.append(result)
                                    st.warning(f"Error processing {result['file']}: {result['error']}")

                            except Exception as e:
                                # This catches exceptions raised by future.result() if any (helper should catch most)
                                # or exceptions in the UI update logic above
                                error_msg = f"Critical error for {os.path.basename(file_path)}: {e}"
                            ...
                            # --- UI Updates (Common for success/fail) ---
                            # 更新日志显示
                            log_text = "**已处理的文件:**\n\n"
